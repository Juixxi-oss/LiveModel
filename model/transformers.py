# adapt from the troch.nn.transformer module
...
class TransformerEncoder(Module):
    ...
    def forward(
            ...
            is_causal: Optional[bool] = None) -> Tuple[Tensor, Tensor]:
        ...
        """
        batch_size, time_steps, features_size = src.shape
        if mask is None and is_causal is True:
            mask = _generate_square_subsequent_mask(time_steps, src.device, torch.float32)
        ...
        attn_maps = torch.stack(attn_maps, dim=1).mean(dim=1)
        # print('all_layers_attn:', all_layers_attn.shape)
...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        x = src
        if self.norm_first:
            _x, attn_map = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, is_causal=is_causal)
            x = x + _x
            x = x + self._ff_block(self.norm2(x))
        else:
            _x, attn_map = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)
            x = self.norm1(x + _x)
