...
class MultiViewTimeFusionRegressorTrainer(BaseTrainer):
    def __init__(self, **kwargs):
        ...
        print('[*] MultiViewTimeFusionRegressorTrainer initialized.')
    ...
    def init_optimizer(self) -> None:
        ...
        print(f'    view fusion lr      : {self.lr}')
    ...
    def _train(self, fold_idx: int, dataloader_lst: list[DataLoader]) -> float:
        ...
        print('-' * 80)
        print(
            f'[*] Start training MultiViewTimeFusionRegressor | '
    ...
    def load_train_model(self, path: str, fold_idx: int) -> None:
        """
        Load full MultiViewTimeFusion checkpoint if provided.
        Otherwise, load pretrained ViewFusionRegressor components.
        """
        if path is not None:
            print(f'[*] Loading MultiViewTimeFusion initial model from: {path}')
            ...
            print('[*] Full MultiViewTimeFusion model loaded.')
            return

        fold_no = fold_idx + 1

        _name = f'{self.model_family}_{self.backbone_type}'
        _prefix = f'file/ViewFusionRegressor/{_name}/model/ViewFusionRegressor_{_name}'

        view_model_path = f'{_prefix}_{self.grade_type}_{fold_no}.pt'

        print('[*] load_model_path is None.')
        print('[*] Loading pretrained ViewFusionRegressor components:')
        print(f'    ViewFusion model: {view_model_path}')
        ...
        try:
            ...
            self.model.extractor_1.load_state_dict(
                _strip(state, 'extractor_1.'),
                strict=True
            )
            self.model.extractor_2.load_state_dict(
                _strip(state, 'extractor_2.'),
                strict=True
            )
            self.model.extractor_3.load_state_dict(
                _strip(state, 'extractor_3.'),
                strict=True
            )
            self.model.extractor_4.load_state_dict(
                _strip(state, 'extractor_4.'),
                strict=True
            )
            self.model.cs_fusion.load_state_dict(
                _strip(state, 'cs_fusion.'),
                strict=True
            )

            try:
                self.model.regressor.load_state_dict(
                    _strip(state, 'regressor.'),
                    strict=True
                )
                print('[*] ViewFusion extractor, fusion, and regressor loaded successfully.')
            except RuntimeError as e:
                print('[!] ViewFusion regressor shape mismatch. Loaded extractor and fusion only.')
                print(f'    {e}')

        except FileNotFoundError:
            print('[!] ViewFusionRegressor checkpoint not found. Training from current initialization.')

        except Exception as e:
            print('[!] Failed to load ViewFusionRegressor components. Training from current initialization.')
            print(f'    Error: {e}')
    ...
    def train_evaluate(self) -> None:
        ...
        print('=' * 80)
        print('[*] Start MultiViewTimeFusionRegressor training/evaluation')
        ...
        print('[*] MultiViewTimeFusionRegressor training/evaluation finished.')