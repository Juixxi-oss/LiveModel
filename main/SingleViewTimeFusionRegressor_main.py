...
from trainer.SingleViewTimeFusionRegressorTrainer import SingleViewTimeFusionRegressorTrainer
from util import getAllTransforms


def parse_args():
    parser = argparse.ArgumentParser(description='Train SingleViewTimeFusionRegressor.')
    ...
    parser.add_argument('--task_type', type=str, required=True, choices=['evaluator', 'predictor'])
    parser.add_argument('--grade_type', type=str, required=True)
    parser.add_argument('--photo_type', type=str, required=True, choices=['D', 'C', 'N', 'P'])
    ...
    parser.add_argument('--result_root', type=str, default='file/SingleViewTimeFusionRegressor')
...
def main():
    ...
    save_model_path = os.path.join(
        ...
        name,
        'model',
        f'SingleViewTimeFusionRegressor_'
        f'{args.task_type}_{name}_{args.grade_type}_photo{args.photo_type}_'
    )
    ...
    default_model_path = os.path.join(
        ...
        'model',
        f'SingleViewTimeFusionRegressor_'
        f'{args.task_type}_{name}_{args.grade_type}_photo{args.photo_type}_default.pt'
    )
    ...
    model = SingleViewTimeFusionRegressor(
        args.task_type,
        args.grade_type,
        args.model_family,
        args.backbone_type,
    ...
    trainer = SingleViewTimeFusionRegressorTrainer(
        model=model,
